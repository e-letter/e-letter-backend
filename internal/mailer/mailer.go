...
type Config struct {
	...
	FromAddress string // EMAIL_FROM (default: SiPena <noreply@eletter.dedyn.io>)
}
...
func (m *resendMailer) SendOTP(toEmail, otp string, expiresAt time.Time) error {
	...
	if toEmail == "" {
		return fmt.Errorf("surel tujuan OTP kosong")
	}
	if otp == "" {
	...
	fromAddress := m.cfg.FromAddress
	if fromAddress == "" {
		fromAddress = "SiPena <noreply@sipena-smkn2.dedyn.io>"
	}

	subject := "Kode OTP Reset Password - SiPena"
	body := buildOTPEmail(toEmail, otp, expiresAt)

	resp, err := m.sender.Send(&resend.SendEmailRequest{
		From:    fromAddress,
		To:      []string{toEmail},
		Subject: subject,
		Html:    body,
	...
	if err != nil {
		return fmt.Errorf("gagal mengirim surel OTP: %w", err)
	}
	log.Printf("[Mailer] OTP sent to %s, ResendID=%s, From=%s", toEmail, resp.Id, fromAddress)
	return nil
}
