...
func (r *letterRepository) CreateLetter(userID int, req domain.LetterCreateRequest) (int, error) {
	...
	effectiveDate := resolveRequestDate(req.RequestDate, req.StartTime, req.EndTime)
	var durationDays int
	var requestDateEnd *string
	if req.RequestDateEnd != "" {
		requestDateEnd = &req.RequestDateEnd
		if startDate, err := time.Parse("2006-01-02", effectiveDate); err == nil {
			if endDate, err := time.Parse("2006-01-02", req.RequestDateEnd); err == nil {
	...
	res, err := tx.Exec(
		`INSERT INTO requests (request_number, academic_year_id, request_type_id, requester_user_id, reason, start_time, end_time, request_date, request_date_end, duration_days, submitted_at, status, current_step)
		 VALUES (?,?,?,?,?,?,?,?,?,?,NOW(),'pending',1)`,
		requestNumber, academicYearID, req.TypeID, userID, req.Description, req.StartTime, req.EndTime, effectiveDate, requestDateEnd, durationDays,
	)
	if err != nil {
		return 0, err
	}
	...
	startTime := extractTimeOnly(req.StartTime)
	endTime := extractTimeOnly(req.EndTime)
	firstPendingStep, firstPendingApproverUserID, err := r.createApprovalSteps(tx, int64(requestID), req.TypeID, userID, int64(academicYearID), startTime, endTime, effectiveDate)
	if err != nil {
...
func (r *letterRepository) createApprovalSteps(tx *sql.Tx, requestID int64, requestTypeID int, userID int, academicYearID int64, requestStartTime, requestEndTime, requestDate string) (int, *int64, error) {
	rows, err := tx.Query(
	...
	var tuTeacherID sql.NullInt64
	{
		var tid int64
		if err := tx.QueryRow(
			`SELECT teacher_id FROM teacher_roles WHERE role_name = 'tu' AND status = 'active' LIMIT 1`,
		).Scan(&tid); err == nil {
			tuTeacherID = sql.NullInt64{Int64: tid, Valid: true}
		}
	}
	...
	for _, t := range templates {
		...
		case "guru_mapel":
			if classID.Valid {
				tid, sid := r.findGuruMapelSchedule(tx, classID.Int64, academicYearID, requestDate, requestStartTime, requestEndTime)
		...
		case "tu":
			if tuTeacherID.Valid {
				tid := tuTeacherID.Int64
				s.ApproverTeacherID = &tid
			}
...
func (r *letterRepository) findGuruMapelSchedule(tx *sql.Tx, classID, academicYearID int64, requestDate, startTime, endTime string) (*int64, *int64) {
	...
	err = tx.QueryRow(
		`SELECT teacher_id, id FROM schedules
		 ...
		 LIMIT 1`, classID, academicYearID, day, endTime, startTime).Scan(&teacherID, &scheduleID)
...
func (r *letterRepository) ListLettersForUser(userID int, typeKey string, page, limit int) (*domain.PaginatedLetterResponse, error) {
	...
	rows, err := r.db.Query(`
		SELECT r.id, MAX(rt.label) AS label, r.reason, r.status, r.request_date, r.created_at, r.updated_at, r.start_time, r.end_time,
		       ...
		       MAX(COALESCE(
		         (SELECT full_name FROM student_profiles WHERE user_id = r.requester_user_id),
		         (SELECT full_name FROM teacher_profiles WHERE user_id = r.requester_user_id),
		         (SELECT full_name FROM principal_profiles WHERE user_id = r.requester_user_id), ''
		       )) AS applicant_name
		FROM requests r
		JOIN request_types rt ON rt.id = r.request_type_id
...
func scanLetterRows(rows *sql.Rows) ([]domain.LetterListItem, error) {
	...
	for rows.Next() {
		var (
			id, title, description, status, fullName, className, nisn, email, applicantName string
			requestDate, createdAt, updatedAt                                               time.Time
			startTime, endTime                                                              sql.NullString
		)
		var idInt int
		if err := rows.Scan(&idInt, &title, &description, &status, &requestDate, &createdAt, &updatedAt, &startTime, &endTime, &fullName, &className, &nisn, &email, &applicantName); err != nil {
			return nil, err
		}
...
func extractTimeOnly(s string) string {
	...
	} else if idx := strings.Index(s, " "); idx > 0 && idx+1 < len(s) {
		s = strings.TrimSpace(s[idx+1:])
	}
...
func (r *letterRepository) ListPrincipalApprovals(userID int, typeKey string, page, limit int) (*domain.PaginatedLetterResponse, error) {
	offset := (page - 1) * limit
	...
	if err != nil {
		if err == sql.ErrNoRows {
			return &domain.PaginatedLetterResponse{Data: []domain.LetterListItem{}, CurrentPage: page, TotalPages: 0, TotalItems: 0}, nil
		}
		return nil, err
	}
	...
	rows, err := r.db.Query(fmt.Sprintf(`
		SELECT
			r.id,
			COALESCE(rt.label, 'Surat') AS label,
			...
			COALESCE(sp.student_code, tp.employee_code, '-') AS nisn,
