...
func ValidateApprovalStep(tx *sql.Tx, requestID int, stepNo int, approverUserID int) (int64, int64, string, bool, int64, error) {
	...
	case "tu":
		...
		if err = tx.QueryRow(`
			SELECT EXISTS(
				SELECT 1 FROM teacher_roles
				WHERE teacher_id = ? AND role_name = 'tu' AND status = 'active'
			)
		`, callerTeacherID.Int64).Scan(&hasTuRole); err != nil {
			return 0, 0, "", false, 0, fmt.Errorf("check tu role: %w", err)
		}
		if !hasTuRole {
			return 0, 0, "", false, 0, errors.New("Anda tidak memiliki peran aktif 'tu' untuk menyetujui langkah ini.")
		}
