...
func (r *permissionRepository) ListAll(startDate, endDate, search, status, typeKey string, page, limit int) (*domain.PaginatedPermissions, error) {
	...
	listSQL := `
		SELECT r.id, r.request_type_id, r.request_number, r.requester_user_id, r.reason, r.request_date, r.start_time, r.end_time, r.status, r.current_step, r.created_at, r.updated_at,
		       CASE
		         WHEN rt.code = 'dispensasi' THEN COALESCE((
		           SELECT GROUP_CONCAT(DISTINCT sp_d.full_name SEPARATOR ', ')
		           FROM request_students rs_d
		           JOIN student_profiles sp_d ON rs_d.student_id = sp_d.id
		           WHERE rs_d.request_id = r.id
		         ), '')
		         ELSE COALESCE(sp.full_name, '')
		       END AS student_name,
		       CASE
		         WHEN rt.code = 'dispensasi' THEN COALESCE((
		           SELECT GROUP_CONCAT(DISTINCT c_d.class_name SEPARATOR ', ')
		           FROM request_students rs_d
		           JOIN student_profiles sp_d ON rs_d.student_id = sp_d.id
		           JOIN student_class_enrollments sce_d ON sce_d.student_id = sp_d.id AND sce_d.is_active = 1
		           JOIN classes c_d ON c_d.id = sce_d.class_id
		           WHERE rs_d.request_id = r.id
		         ), '-')
		         ELSE COALESCE(c.class_name, '-')
		       END AS class_name,
	...
	listSQL := `
		SELECT r.id,
		       ...
		       MAX(CASE
		         ...
		         ELSE COALESCE(sp_req.full_name, sp_target.full_name)
		       END) AS student_name,
		       MAX(CASE
		         WHEN rt.code = 'dispensasi' THEN COALESCE((
		           SELECT GROUP_CONCAT(DISTINCT c_d.class_name SEPARATOR ', ')
		         ...
		         ELSE COALESCE(c_req.class_name, c_target.class_name, '-')
		       END) AS class_name,
