...
func (r *authRepository) CreateUser(roleID int, email, passwordHash, status string) (int, error) {
	...
	if err != nil {
		...
		if errors.As(err, &mysqlErr) && mysqlErr.Number == 1062 {
			return 0, fmt.Errorf("Surel sudah terdaftar")
		}
		return 0, err
	}
...
func (r *authRepository) LogLoginAttempt(attempt domain.LoginAttempt) error {
	...
	if attempt.Description != nil {
		description = *attempt.Description
	} else if !attempt.Success {
		activityType = "login_failed"
		if attempt.EmailAttempted != nil {
			description = "Login gagal untuk surel/identifier: " + *attempt.EmailAttempted
		} else {
...
func (r *authRepository) CreatePasswordResetToken(userID int, otpCode string, expiresAt time.Time, ip string) error {
	...
	log.Printf("[DB] Insert OTP: userID=%d otpCode_len=%d otpCode_prefix=%s", userID, len(otpCode), otpCode[:min(8, len(otpCode))])
	_, err := r.db.Exec(
...
func (r *authRepository) UpdatePassword(userID int, passwordHash string) error {
	...
	if _, err := tx.Exec(`UPDATE users SET password_hash = ?, password_changed_at = NOW() WHERE id = ?`, passwordHash, userID); err != nil {
		return err
	}
