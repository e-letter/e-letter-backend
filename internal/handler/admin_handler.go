...
func (h *AdminHandler) CreateClass(c *gin.Context) {
	...
	adminUserID := int64(toIntFromContext(c, "userId"))
	if err := h.svc.CreateClass(body.ClassName, body.MajorID, adminUserID, c.ClientIP(), c.Request.UserAgent()); err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
...
func (h *AdminHandler) CreateMajor(c *gin.Context) {
	...
	response.Success(c, http.StatusCreated, "Jurusan berhasil dibuat", nil)
}
...
func (h *AdminHandler) UpdateMajor(c *gin.Context) {
	...
	response.Success(c, http.StatusOK, "Jurusan berhasil diperbarui", nil)
}
...
func (h *AdminHandler) DeleteMajor(c *gin.Context) {
	...
	response.Success(c, http.StatusOK, "Jurusan berhasil dihapus", nil)
}
...
func (h *AdminHandler) GetAuditLogs(c *gin.Context) {
	...
	logs, total, totalPages, currentPage, err := h.svc.GetAuditLogs(activityType, search, page, limit)
	if err != nil {
		response.Error(c, http.StatusInternalServerError, err.Error())
		return
	}
