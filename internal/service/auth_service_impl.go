...
func (s *authService) Register(req domain.RegisterRequest) (int, string, error) {
	...
	if req.Role == "guru" || req.Role == "teacher" {
		if req.Token == "" {
			return 0, "", errors.New("Token diperlukan untuk pendaftaran guru")
		}

		tokenRec, err := s.repo.GetRegistrationToken(req.Token)
		if err != nil {
			return 0, "", errors.New("Token tidak valid")
		}
		if tokenRec.UsageLimit != nil && tokenRec.UsedCount >= *tokenRec.UsageLimit {
			return 0, "", errors.New("Batas penggunaan token tercapai")
		}
		roleID = 2
	}
	...
	if roleID == 2 {
		if err := s.repo.IncrementRegistrationTokenUsage(req.Token); err != nil {
			return 0, "", err
		}
	}
...
func (s *authService) Login(req domain.LoginRequest, ip, userAgent string) (*domain.User, string, string, error) {
	...
	accessClaims := utils.TokenClaims{
		UserID: user.ID,
		Email:  user.Email,
		Role:   user.RoleID,
		Type:   "access",
	...
	accessToken, err := utils.GenerateToken(s.jwtSecret, accessClaims)
	if err != nil {
		return nil, "", "", err
	}
	...
	refreshClaims.Exp = time.Now().Add(s.refreshExpiry).Unix()
	refreshToken, err := utils.GenerateToken(s.jwtSecret, refreshClaims)
	if err != nil {
		return nil, "", "", err
	}
...
func (s *authService) Refresh(refreshToken string) (string, string, error) {
	...
	newAccessClaims := utils.TokenClaims{
	...
	newAccessToken, err := utils.GenerateToken(s.jwtSecret, newAccessClaims)
	if err != nil {
		return "", "", err
	}
	...
	newRefreshClaims.Exp = time.Now().Add(s.refreshExpiry).Unix()
	newRefreshToken, err := utils.GenerateToken(s.jwtSecret, newRefreshClaims)
	if err != nil {
		return "", "", err
	}
