...
func (s *authService) Register(req domain.RegisterRequest) (int, string, error) {
	...
	if isTeacher {
		if !strings.HasSuffix(strings.ToLower(strings.TrimSpace(req.Email)), "@guru.smk.belajar.id") {
			return 0, "", errors.New("Pendaftaran guru hanya diizinkan menggunakan surel @guru.smk.belajar.id")
		}
	}

	existing, err := s.repo.GetUserByEmailAnyStatus(req.Email)
	if err != nil {
		return 0, "", fmt.Errorf("gagal memeriksa surel: %w", err)
	}
	if existing != nil {
		if existing.Status == "pending" {
			return 0, "", errors.New("Surel sudah terdaftar dan sedang menunggu persetujuan admin")
		}
		return 0, "", errors.New("Surel sudah terdaftar")
	}
...
func (s *authService) ForgotPassword(email, ip string) error {
	if strings.TrimSpace(email) == "" {
		return errors.New("Surel diperlukan")
	}

	user, err := s.repo.GetUserByEmail(email)
	if err != nil {
		return errors.New("Gagal memeriksa surel")
	}
	if user == nil {
		return errors.New("Surel tidak ditemukan")
	}
	...
	log.Printf("[ForgotPassword] Sending OTP to email=%s otp=%s", email, otp)
	if err := s.mailer.SendOTP(email, otp, expiresAt); err != nil {
		errMsg := fmt.Sprintf("[ForgotPassword] SendOTP error: %v", err)
		log.Print(errMsg)
		fmt.Fprintln(os.Stderr, errMsg)
		return fmt.Errorf("gagal kirim OTP: %w", err)
	}
	log.Printf("[ForgotPassword] OTP sent successfully to %s", email)
...
func (s *authService) VerifyOTP(email, otp string) error {
	if strings.TrimSpace(email) == "" || strings.TrimSpace(otp) == "" {
		return errors.New("Surel dan OTP diperlukan")
	}
...
func (s *authService) ResetPassword(email, otp, newPassword string) error {
	if strings.TrimSpace(email) == "" || strings.TrimSpace(otp) == "" || strings.TrimSpace(newPassword) == "" {
		return errors.New("Surel, OTP, dan kata sandi baru diperlukan")
	}
