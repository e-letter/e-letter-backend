...
func (s *adminService) GetStats() (map[string]int, error) {
	...
	for _, r := range rows {
		...
		if err := s.db.QueryRow(r.query).Scan(&count); err != nil {
			return nil, fmt.Errorf("failed to fetch %s: %w", r.key, err)
		}
		stats[r.key] = count
	}
	return stats, nil
...
func (s *adminService) GetUsers(role, status, search string, page, pageSize int) ([]domain.AdminUserListItem, int, int, int, error) {
	...
	if search != "" {
		baseQuery += " AND (u.email LIKE ? OR tp.full_name LIKE ? OR sp.full_name LIKE ? OR ap.full_name LIKE ? OR pp.full_name LIKE ?)"
		s := "%" + search + "%"
	...
	selectQuery := "SELECT u.id, u.email, u.role, u.status, COALESCE(tp.full_name, sp.full_name, ap.full_name, pp.full_name, '') as full_name " + baseQuery + " ORDER BY u.id DESC LIMIT ? OFFSET ?"
...
func (s *adminService) CreateUser(fullName, email, role, status, password string, adminUserID int64, ip, userAgent string) (map[string]any, error) {
	...
	if usesUsername {
		...
		if err == nil {
			return nil, fmt.Errorf("Surel sudah terdaftar")
		}
...
func (s *adminService) AdminDeleteLetter(id int, adminUserID int64, ip, userAgent string) (map[string]any, error) {
	...
	err = tx.QueryRow(
		`SELECT r.requester_user_id, r.request_number, COALESCE(rt.code, ''), r.status FROM requests r LEFT JOIN request_types rt ON rt.id = r.request_type_id WHERE r.id = ? AND r.deleted_at IS NULL FOR UPDATE`,
		id,
...
func (s *adminService) UpdateAcademicYear(id string, name *string, isActive *bool, startDate, endDate *string, adminUserID int64, ip, userAgent string) error {
	...
	if isActive != nil && *isActive {
		if _, err := tx.Exec(`UPDATE academic_years SET is_active = 0`); err != nil {
			return err
		}
	}

	if _, err := tx.Exec(`UPDATE academic_years SET name = COALESCE(?, name), is_active = COALESCE(?, is_active), start_date = COALESCE(?, start_date), end_date = COALESCE(?, end_date) WHERE id = ?`,
		name, isActive, startDate, endDate, id); err != nil {
		return err
	}
...
func (s *adminService) CreateClass(className string, majorID, gradeLevel, academicYearID int, adminUserID int64, ip, userAgent string) error {
	_, err := s.db.Exec(`INSERT INTO classes (class_name, major_id, grade_level, academic_year_id) VALUES (?, ?, ?, ?)`, className, majorID, gradeLevel, academicYearID)
	if err != nil {
		return err
	}
...
func (s *adminService) UpdateClass(id, className string, majorID int, adminUserID int64, ip, userAgent string) error {
	_, err := s.db.Exec(`UPDATE classes SET class_name = ?, major_id = ? WHERE id = ?`, className, majorID, id)
	if err != nil {
		return err
	}
...
func (s *adminService) CreateMajor(name, short string, adminUserID int64, ip, userAgent string) error {
	...
	utils.LogActivity(s.db, adminUserID, "create_major", "Admin membuat konsentrasi keahlian: "+name, ip, userAgent)
	return nil
}
...
func (s *adminService) UpdateMajor(id, name, short string, adminUserID int64, ip, userAgent string) error {
	...
	utils.LogActivity(s.db, adminUserID, "update_major", "Admin memperbarui konsentrasi keahlian id="+id, ip, userAgent)
	return nil
}
...
func (s *adminService) DeleteMajor(id string, adminUserID int64, ip, userAgent string) error {
	...
	utils.LogActivity(
		...
		adminUserID,
		"delete_major",
		fmt.Sprintf("Admin menghapus konsentrasi keahlian id=%s (rows_affected=%d)", id, affected),
		ip,
		userAgent,
	)
	return nil
...
func (s *adminService) GetSchedules() ([]domain.AdminScheduleItem, error) {
	rows, err := s.db.Query(`
		SELECT s.id,
		       COALESCE(s.academic_year_id,0), COALESCE(s.class_id,0),
		       COALESCE(s.subject_id,0), COALESCE(s.teacher_id,0),
		       COALESCE(s.day_of_week,''), COALESCE(s.start_time,''), COALESCE(s.end_time,''),
		       COALESCE(s.is_active,1),
		       COALESCE(c.class_name,''), COALESCE(sub.name,''),
	...
	var items []domain.AdminScheduleItem
	for rows.Next() {
		var item domain.AdminScheduleItem
		if err := rows.Scan(&item.ID, &item.AcademicYearID, &item.ClassID, &item.SubjectID, &item.TeacherID,
			&item.DayOfWeek, &item.StartTime, &item.EndTime, &item.IsActive,
			&item.ClassName, &item.SubjectName, &item.TeacherName); err != nil {
			return nil, err
		}
		items = append(items, item)
	}
...
func (s *adminService) CreateSchedule(academicYearID, classID, subjectID, teacherID int, dayOfWeek, startTime, endTime string, adminUserID int64, ip, userAgent string) error {
	_, err := s.db.Exec(
		`INSERT INTO schedules (academic_year_id, class_id, subject_id, teacher_id, day_of_week, start_time, end_time) VALUES (?, ?, ?, ?, ?, ?, ?)`,
		academicYearID, classID, subjectID, teacherID, dayOfWeek, startTime, endTime,
	)
	if err != nil {
		return err
	}
	utils.LogActivity(s.db, adminUserID, "create_schedule", "Admin membuat jadwal: kelas="+strconv.Itoa(classID)+" mapel="+strconv.Itoa(subjectID), ip, userAgent)
	return nil
}

func (s *adminService) UpdateSchedule(id string, academicYearID, classID, subjectID, teacherID int, dayOfWeek, startTime, endTime string, adminUserID int64, ip, userAgent string) error {
	_, err := s.db.Exec(
		`UPDATE schedules SET academic_year_id=?, class_id=?, subject_id=?, teacher_id=?, day_of_week=?, start_time=?, end_time=? WHERE id=?`,
		academicYearID, classID, subjectID, teacherID, dayOfWeek, startTime, endTime, id,
	)
	if err != nil {
		return err
	}
	utils.LogActivity(s.db, adminUserID, "update_schedule", "Admin memperbarui jadwal id="+id, ip, userAgent)
	return nil
}

func (s *adminService) DeleteSchedule(id string, adminUserID int64, ip, userAgent string) error {
	res, err := s.db.Exec(`DELETE FROM schedules WHERE id = ?`, id)
	if err != nil {
		return err
	}
	affected, _ := res.RowsAffected()
	utils.LogActivity(s.db, adminUserID, "delete_schedule", fmt.Sprintf("Admin menghapus jadwal id=%s (rows_affected=%d)", id, affected), ip, userAgent)
	return nil
}
...
func (s *adminService) BulkPromoteStudents(promotions []domain.BulkPromotionItem, adminUserID int64, ip, userAgent string) error {
	if len(promotions) == 0 {
		return fmt.Errorf("Tidak ada data kenaikan kelas")
	}

	tx, err := s.db.Begin()
	if err != nil {
		return fmt.Errorf("Gagal memulai transaksi database")
	}
	defer tx.Rollback()
	...
	for _, p := range promotions {
		...
		_, err := tx.Exec(
			`UPDATE student_class_enrollments
			 SET is_active = 0, promotion_status = ?, promotion_decided_at = NOW(), promotion_decided_by = ?
			 WHERE id = ? AND is_active = 1`,
			promotionStatus, adminUserID, p.EnrollmentID,
		)
		if err != nil {
			return fmt.Errorf("Gagal menonaktifkan enrollment id=%d: %s", p.EnrollmentID, err.Error())
		}

		_, err = tx.Exec(
			`INSERT INTO student_class_enrollments (student_id, class_id, academic_year_id, is_active, promotion_status, promotion_note)
			 VALUES (?, ?, ?, 1, 'active', ?)`,
			p.StudentID, p.TargetClassID, p.AcademicYearID, promotionStatus,
		)
		if err != nil {
			return fmt.Errorf("Gagal membuat enrollment baru untuk student_id=%d: %s", p.StudentID, err.Error())
		}
	}

	if err := tx.Commit(); err != nil {
		return fmt.Errorf("Gagal menyimpan perubahan kenaikan kelas")
	}
	...
	utils.LogActivity(
		...
		fmt.Sprintf("Admin memproses kenaikan kelas untuk %d siswa", len(promotions)),
		ip,
		userAgent,
	)
	return nil
...
func (s *adminService) GetAuditLogs(activityType, search string, page, limit int) ([]domain.AuditLogItem, int, int, int, map[string]int, error) {
	if page < 1 {
		page = 1
	}
	if limit < 1 {
	...
	typeRows, err := s.db.Query(`SELECT activity_type, COUNT(*) FROM activity_logs`+whereSQL+` GROUP BY activity_type`, args...)
	if err == nil {
		defer typeRows.Close()
		for typeRows.Next() {
			var at string
			var cnt int
			if typeRows.Scan(&at, &cnt) == nil {
				typeCounts[at] = cnt
			}
		}
	}

	return logs, total, totalPages, page, typeCounts, nil
}
