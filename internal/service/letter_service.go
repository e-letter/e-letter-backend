...
func (s *letterService) Create(userID int, req domain.LetterCreateRequest) (int, error) {
	...
	if req.IsMultiDay() {
		startDate, err := time.Parse("2006-01-02", effectiveDate)
		if err != nil {
			return 0, errors.New("Format tanggal mulai tidak valid")
		}
		endDate, err := time.Parse("2006-01-02", req.RequestDateEnd)
		if err != nil {
			return 0, errors.New("Format tanggal selesai tidak valid")
		}
		if endDate.Before(startDate) {
			return 0, errors.New("Tanggal selesai harus setelah atau sama dengan tanggal mulai")
		}
...
func (s *letterService) ListPrincipalApprovals(userID int, typeKey string, page, limit int) (*domain.PaginatedLetterResponse, error) {
	isPrincipal, err := s.repo.IsActivePrincipal(userID)
	if err != nil {
		return nil, err
	}
	if !isPrincipal {
		return nil, errors.New("forbidden: not a principal")
	}
	return s.repo.ListPrincipalApprovals(userID, typeKey, page, limit)
}
