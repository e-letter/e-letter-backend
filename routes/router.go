...
func SetupRouter(
	...
	api := r.Group("/api/v1")
	{
		...
		protected.Use(middleware.RequireAccessToken(cfg.JWT.Secret))
		{
			...
			protected.GET("/letters/teacher/pending", middleware.RequireRole("teacher"), rateLimiter.ReadRateLimiter(), letterHandler.TeacherPending)
			protected.GET("/letters/teacher", middleware.RequireRole("teacher"), rateLimiter.ReadRateLimiter(), letterHandler.TeacherLetters)
			protected.GET("/letters/dispensasi", middleware.RequireRole("teacher"), rateLimiter.ReadRateLimiter(), letterHandler.GeneralDispensasi)
			protected.GET("/letters/general/dispensasi", middleware.RequireRole("teacher"), rateLimiter.ReadRateLimiter(), letterHandler.GeneralDispensasi)
			protected.GET("/letters/kepsek/pending", middleware.RequireRole("kepala_sekolah"), rateLimiter.ReadRateLimiter(), letterHandler.KepsekPending)
			protected.GET("/letters/kepsek/stats", middleware.RequireRole("kepala_sekolah"), rateLimiter.ReadRateLimiter(), letterHandler.KepsekStats)
